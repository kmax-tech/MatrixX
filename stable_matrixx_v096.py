...
#this the matrixx solver for stable semantics
...
class Semantics_Dealer():
    ...
    def parse_arguments(self):
        ...
        #debug part for more detailed output
        if self.debug:
    ...
    def control_calculations(self):
        ...
        if self.semantics == "CE-ST":
            after_eval = self.sem_check_CE_ST

        #check parameter in sceptical interpretations
        elif self.semantics == "DS-ST":
            after_eval = self.sem_check_DS_ST

        elif self.semantics == "DC-ST": #credulously accepted
            loop_eval = self.sem_check_DC_ST_loop
            after_eval = self.sem_check_DC_ST_after

        elif self.semantics == "SE-ST": #one extension
            loop_eval = self.sem_check_SE_ST_loop
            after_eval = self.sem_check_SE_ST_loop_after
    ...
    def sem_check_CE_ST(self): #give number of all extensions
        print(len(self.Ctrl_Calc.total_interpretations))

    def sem_check_DS_ST(self): #check sceptical interpretations
...
class Control_Calculation():
    ...
    def final_interpretation_manager(self,Calc_Object):
        self.total_interpretations.append(Calc_Object.interpretations) #update final interpretations

    def eval_Calc_Object(self,Node_Structure):
        if len(Node_Structure.offensive_nodes) == 0 and len(Node_Structure.defensive_nodes) == 0:
            self.final_interpretation_manager(Node_Structure) #append the interpretations

        elif len(Node_Structure.offensive_nodes) != 0:
...
class Defensive_Ripper():
    ...
    def eval_defense(self):
        ...
        if defensive_node[1] == 0:  #an unattacked object should always be chosen, no forking occurs
            ...
            #self.output[0].complete_interpretations.append(deepcopy(self.output[0].interpretations))
        else:
            x = self.use_node(self.new_objects[0], defensive_node[0])
...
class Offensive_Ripper():
    ...
    def offense_eval(self):
        ...
        #just erasing of current buffer information
        if len(self.Node_Structure.current_interpretation_node) == 0:
            #the single buffer node in this case also selected for deletion of defensive properties
            self.clean_offensive_attack_nodes()
            self.clean_offense_nodes()
            #no dealing with defended property, because it is not affected in this configuration
...
class Structure_Preprocessing():
    ...
    def init_calc_structure(self):
        ...
        for self_att_node in self.Init_Node_Structure.self_attacking_nodes:
            ...
            #for stable are actively selected
